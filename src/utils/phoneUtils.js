...
function parsePhoneAndCountryCode(countryCodeInput, phoneInput) {
  if (!countryCodeInput && !phoneInput) return { countryCode: null, phone: null };

  let cc = (countryCodeInput || '').trim();
  let ph = (phoneInput || '').trim();
  ...
  let clean = combined.replace(/[\s()-]/g, '');
  if (!clean.startsWith('+') && clean.length > 0) {
    clean = '+' + clean;
  }

  for (let len of [4, 3, 2]) { // 3-digit, 2-digit, 1-digit prefixes (+ included)
    if (clean.length > len) {
      const prefix = clean.substring(0, len);
      if (KNOWN_COUNTRY_CODES.has(prefix)) {
        return {
          countryCode: prefix,
          phone: clean.substring(len),
        };
      }
    }
  }

  const ccMatch = cc.match(/^(\+[1-9]\d{0,2})$/);
  if (ccMatch && ph) {
    return { countryCode: ccMatch[1], phone: ph.replace(/\D/g, '') };
  }

  const genMatch = clean.match(/^(\+[1-9]\d{0,2})(\d{6,14})$/);
  if (genMatch) {
    return { countryCode: genMatch[1], phone: genMatch[2] };
  }

  return { countryCode: cc || null, phone: ph || null };
}
